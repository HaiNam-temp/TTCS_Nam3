...
def scrape_cellphones_products(product_name: str) -> List[Dict]:
    ...
    try:
        ...
        logger.info("[cellphones][entry] product_name=%s search_url=%s", product_name, search_url)
        raw_results = scrape(search_url, limit=5)  # Giới hạn 5 sản phẩm
        logger.info("[cellphones][entry] raw_results_count=%s", len(raw_results))
        ...
        for idx, item in enumerate(raw_results[:5], 1):  # Đảm bảo chỉ lấy 5 sản phẩm
            if not item.get('title'):
                logger.info("[cellphones][normalize] skip idx=%s reason=missing_title", idx)
                continue
        ...
        logger.info("[cellphones][entry] normalized_products_count=%s", len(products))
        
        return products
        
    except Exception as e:
        logger.exception("[cellphones][entry] failed product_name=%s", product_name)
...
def scrape(search_url, limit=None):
    results = []
    logger.info("[cellphones][scrape] start url=%s limit=%s", search_url, limit)
    ...
    with sync_playwright() as p:
        ...
        logger.info("[cellphones][scrape] page_loaded url=%s", search_url)
        ...
        for sel in selectors:
            try:
                ...
                found = True
                logger.info("[cellphones][scrape] selector_ready selector=%s", sel)
                break
            except Exception:
                continue
        if not found:
            logger.info("[cellphones][scrape] no_primary_selector_found selectors=%s", selectors)
        ...
        for sel in item_selectors:
            ...
            if items:
                logger.info("[cellphones][scrape] item_selector_hit selector=%s count=%s", sel, len(items))
                break
        if not items:
            logger.info("[cellphones][scrape] no_item_container_found fallback=anchors")

        # fallback: anchors that look like product links
        if not items:
            anchors = page.query_selector_all('a.product-item-link, a[href$=".html"]')
            logger.info("[cellphones][scrape] anchor_candidates_count=%s", len(anchors))
            for a in anchors:
                ...
                if img_el:
                    img_src = img_el.get_attribute('src') or img_el.get_attribute('data-src') or img_el.get_attribute('data-lazy-src')
                    if img_src:
                        img = urljoin(search_url, img_src)
        ...
        browser.close()
    logger.info("[cellphones][scrape] end results_count=%s", len(results))
    return results
