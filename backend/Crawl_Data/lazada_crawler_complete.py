...
class LazadaCrawler:
    ...
    def create_web_driver(self, url: str) -> WebDriver:
        ...
        logger.info("[lazada][driver] init url=%s", url)
        ...
        sleep(2)
        logger.info("[lazada][driver] ready url=%s", url)
        return driver
    ...
    def get_product_info_json(self, soup: BeautifulSoup):
        ...
        logger.info(
            "[lazada][parse] selector_counts names=%s prices=%s sold=%s origins=%s ratings=%s reviews=%s",
            len(name_items),
            len(price_items),
            len(sold_items),
            len(product_origin),
        ...
        for index in range(len(name_items)):
            try:
                ...
                if not name:
                    logger.info("[lazada][parse] skip index=%s reason=missing_title", index)
                    continue
                ...
                logger.info("[lazada][parse] skip index=%s reason=exception error=%s", index, e)
    ...
    def crawl_lazada_products(self, product_name: str) -> List[Dict]:
        ...
        try:
            ...
            all_products = []
            logger.info(
                "[lazada][entry] start product_name=%s filtered_keyword=%s",
                product_name,
                filtered_keyword,
            )
            ...
            for page in range(1, 3):  # Crawl tối đa 2 trang
                ...
                logger.info("[lazada][entry] crawl_page page=%s url=%s", page, url)
                
                driver = self.create_web_driver(url)
                html = driver.execute_script("return document.getElementsByTagName('html')[0].innerHTML")
                logger.info("[lazada][entry] html_length page=%s length=%s", page, len(html or ""))
                soup = BeautifulSoup(html, "html.parser")
                
                products = self.get_product_info_json(soup)
                logger.info("[lazada][entry] page_products_count page=%s count=%s", page, len(products))
                ...
                if len(all_products) >= 5:
                    logger.info("[lazada][entry] stop_early reason=reached_limit total=%s", len(all_products))
                    break
            ...
            logger.info("[lazada][entry] end total_products=%s", len(all_products))
            
            return all_products
        
        except Exception as e:
            logger.exception("[lazada][entry] failed product_name=%s", product_name)
