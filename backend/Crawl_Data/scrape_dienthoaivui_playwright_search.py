...
def scrape_dienthoaivui_products(product_name: str) -> List[Dict]:
    ...
    try:
        ...
        logger.info("[dienthoaivui][entry] product_name=%s search_url=%s", product_name, search_url)
        raw_results = scrape(search_url, limit=10)  # Lấy 10 để có đủ data filter
        logger.info("[dienthoaivui][entry] raw_results_count=%s", len(raw_results))
        ...
        for idx, item in enumerate(raw_results, 1):
            ...
            if not item.get('title'):
                logger.info("[dienthoaivui][normalize] skip idx=%s reason=missing_title", idx)
                continue
        ...
        logger.info("[dienthoaivui][entry] normalized_products_count=%s", len(products))
        
        return products
        
    except Exception as e:
        logger.exception("[dienthoaivui][entry] failed product_name=%s", product_name)
...
def scrape(search_url, limit=None):
    results = []
    logger.info("[dienthoaivui][scrape] start url=%s limit=%s", search_url, limit)
    try:
        import platform
        import asyncio
    ...
    with sync_playwright() as p:
        ...
        logger.info("[dienthoaivui][scrape] page_loaded url=%s", search_url)
        ...
        logger.info("[dienthoaivui][scrape] anchor_candidates_count=%s", len(anchors))
        seen = set()
        results_by_anchor = []
        ...
        try:
            ...
            logger.info(
                "[dienthoaivui][scrape] anchors_processed scanned=%s matched=%s",
                len(anchors),
                len(results_by_anchor),
            )
        except Exception:
            pass
        ...
        if results_by_anchor:
            ...
            for sel in item_selectors:
                try:
                    ...
                    if items:
                        logger.info("[dienthoaivui][scrape] item_selector_hit selector=%s count=%s", sel, len(items))
                        break
                except Exception:
                    continue
            if not items:
                logger.info("[dienthoaivui][scrape] no_item_container_found fallback=anchors")
            ...
            if items:
                ...
                anchors = page.query_selector_all('a[href]')
                logger.info("[dienthoaivui][scrape] fallback_anchor_candidates_count=%s", len(anchors))
                for a in anchors:
                    if limit is not None and len(results) >= limit:
                        break
                    try:
    ...
    for r in filtered:
        ...
        if limit is not None and len(final) >= limit:
            break
    logger.info(
        "[dienthoaivui][scrape] end raw=%s filtered=%s final=%s",
        len(results),
        len(filtered),
        len(final),
    )
